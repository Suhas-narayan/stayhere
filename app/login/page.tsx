  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast({
        title: "Success!",
        description: "You've been logged in successfully.",
        variant: "default", // Or keep it green if you prefer
      });

      // --- REDIRECT LOGIC ---
      // Get the intended redirect path from the query parameter
      const redirectPath = searchParams.get('redirect');

      // Redirect to the intended page, or fallback to '/' if no redirect param
      router.push(redirectPath || '/');
      // You could also use router.replace() if you don't want the login page
      // in the browser history after redirection.
      // router.replace(redirectPath || '/');
      // --- END REDIRECT LOGIC ---

    } catch (error: any) { // Catch specific error type if known, else 'any' or 'unknown'
      console.error("Login failed:", error); // Log the full error for debugging
      toast({
        title: "Authentication failed",
        // Use error.message if available and user-friendly, otherwise a generic message
        description: error?.message || "Please check your credentials and try again.",
        variant: "destructive",
      });
      setIsLoading(false); // Ensure loading is set to false on error
    }
    // No finally block needed here as we only want to stop loading on error
    // or let the redirect happen on success (component might unmount)
  };
